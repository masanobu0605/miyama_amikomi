...
def loop1(gamma):
    ...
    result_F = result_F
    return result_F,MF(result_F,gamma),MD(result_F,gamma),MR(result_F,gamma),T(result_F,gamma)

# 解析実行+データ整理
F = np.zeros(gamma_count)
MF = np.zeros(gamma_count)
MD= np.zeros(gamma_count)
MR = np.zeros(gamma_count)
T = np.zeros(gamma_count)
MF_and_MD = np.zeros(gamma_count)
d_gamma = np.zeros(gamma_count)
for i in range(gamma_count):
    F[i],MF[i],MD[i],MR[i],T[i] = loop1(gammalist[i])
    F[i] = round(F[i],5)
    T[i] = round(T[i],5)
    MF[i] = round(MF[i],5)
    MD[i] = round(MD[i],5)
    MR[i] = round(MR[i],5)
    MF_and_MD[i] = MF[i]+MD[i]
...
datasheet[:,7] = MR
datasheet_pd = pd.DataFrame(datasheet,columns=["gammalist [deg]","d_gamma [deg]","F [N]"," T[N]","MF [Nmm]","MD [Nmm]","MF+MD [Nmm]","MR [Nmm]"],)
